...
to_bot_suffix = C(
    r"(( please)? (<:ai_name>|girl|dear|babe|honey|sweet ?heart|darling|ma.?am))$"
)
...
ip.what_time = [
    ...
    C(r"(what('| )?(s|re|is|are|r|was|were|am|will|will be)? )?(the )?(current )?time( is| it)*( now)?( please)?"),
    'clock',
]
